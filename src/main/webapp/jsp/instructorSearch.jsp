...
<body>
	...
	<div id="frameBody">
		<div id="frameBodyWrapper" class="container">
						...
						<div class="form-group">
							<div class="input-group">
								<input type="text" name="searchkey"
									value="<%=InstructorSearchPageData.sanitizeForHtml(data.searchKey)%>"
									title="Search for comment" placeholder="Your search keyword"
									class="form-control" id="searchBox"> <span
						...
						<div class="form-group">
							<ul class="list-inline">
								<li><span data-toggle="tooltip"
									title="Tick the checkboxes to limit your search to certain categories"
									class="glyphicon glyphicon-info-sign"></span></li>
								<li><input id="comments-for-student-check" type="checkbox"
									name="<%=Const.ParamsNames.SEARCH_COMMENTS_FOR_STUDENTS%>"
									value="true" <%=data.isSearchCommentForStudents?"checked":""%>>
									<label for="comments-for-student-check">Comments for
										students</label></li>
								<li><input id="comments-for-responses-check"
									type="checkbox"
									name="<%=Const.ParamsNames.SEARCH_COMMENTS_FOR_RESPONSES%>"
									value="true" <%=data.isSearchCommentForResponses?"checked":""%>>
									<label for="comments-for-responses-check">Comments for
										responses</label></li>
								<li><input id="students-check" type="checkbox"
									name="<%=Const.ParamsNames.SEARCH_STUDENTS%>" value="true"
									<%=data.isSearchForStudents || 
                                (!data.isSearchCommentForStudents && !data.isSearchCommentForResponses)?"checked":""%>>
									<label for="students-check">Students</label></li>
							</ul>
						</div>
					</div>
				</form>
							...
							<li class="list-group-item list-group-item-warning"
								...
								<div id="commentBar-<%=commentIdx%>">
									<span class="text-muted">To <b><%=recipientDisplay%></b>
										on <%=TimeHelper.formatTime(comment.createdAt)%></span> <a
										type="button"
										href="<%=data.getInstructorCommentsLink() + "&" + Const.ParamsNames.COURSE_ID 
                                    + "=" + comment.courseId + "#" + comment.getCommentId()%>"
										target="_blank"
										class="btn btn-default btn-xs icon-button pull-right"
										data-toggle="tooltip" data-placement="top"
										data-original-title="Edit comment in the Comments page"
										style="display: none;"> <span
										class="glyphicon glyphicon-new-window glyphicon-primary"></span>
									</a>
								</div>
			...
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					<div class="row <%=fsIndx == 1?"":"border-top-gray"%>">
						<div class="col-md-2">
							<strong>Session: <%=fsName%> (<%=data.feedbackResponseCommentSearchResultBundle.sessions.get(fsName).courseId%>)
							</strong>
						</div>
						<div class="col-md-10">
							<%
                        ...
                        %>
							<div class="panel panel-info">
								...
								<table class="table">
                                    ...
                                    %>
										<tr>
											<td><b>From:</b> <%=giverName%> <b>To:</b> <%=recipientName%>
											</td>
										</tr>
										<tr>
											<td><strong>Response: </strong><%=responseEntry.getResponseDetails().getAnswerHtml(question.getQuestionDetails())%>
											</td>
										</tr>
										<tr class="active">
											<td>Comment(s):</td>
			...
			<div class="panel panel-info">
				<div class="panel-heading">
					<strong>[<%=currentCourse%>]
					</strong>
				</div>
				<div class="panel-body padding-0">
					<table
						class="table table-responsive table-striped table-bordered margin-0">
						<thead
							class="background-color-medium-gray text-color-gray font-weight-normal">
							<tr>
								<th>Photo</th>
								<th id="button_sortsection-<%=courseIdx%>"
									class="button-sort-none" onclick="toggleSort(this, 1)">Section
									<span class="icon-sort unsorted">
								</th>
								<th id="button_sortteam-<%=courseIdx%>" class="button-sort-none"
									onclick="toggleSort(this, 2)">Team <span
									class="icon-sort unsorted"></th>
								<th id="button_sortname-<%=courseIdx%>" class="button-sort-none"
									onclick="toggleSort(this, 3)">Student Name <span
									class="icon-sort unsorted"></th>
								<th id="button_sortemail-<%=courseIdx%>"
									class="button-sort-none" onclick="toggleSort(this, 4)">Email
									<span class="icon-sort unsorted">
								</th>
								<th>Action(s)</th>
							</tr>
							...
							<tr id="student-c<%=courseIdx%>.<%=studentIdx%>"
								...
								<td id="studentphoto-c<%=courseIdx%>.<%=studentIdx%>">
									<div class="profile-pic-icon-click align-center"
										data-link="<%=data.addUserIdToUrl(student.getPublicProfilePictureUrl())%>">
										<a class="student-profile-pic-view-link btn-link"> View
											Photo</a> <img src="" alt="No Image Given" class="hidden">
									</div>
								</td>
								<td id="studentsection-c<%=courseIdx %>.<%=studentIdx%>"><%=PageData.sanitizeForHtml(student.section)%>
								</td>
								<td id="studentteam-c<%=courseIdx %>.<%=studentIdx%>"><%=PageData.sanitizeForHtml(student.team)%>
								</td>
								<td id="studentname-c<%=courseIdx %>.<%=studentIdx%>"><%=PageData.sanitizeForHtml(student.name)%>
								</td>
								<td id="studentemail-c<%=courseIdx %>.<%=studentIdx%>"><%=PageData.sanitizeForHtml(student.email)%>
								</td>
								<td class="no-print align-center"><a
									class="btn btn-default btn-xs student-view-for-test"
									...
									class="btn btn-default btn-xs student-edit-for-test"
									...
									disabled="disabled" <% } %>> Edit </a> <a
									class="btn btn-default btn-xs student-delete-for-test"
									...
									class="btn btn-default btn-xs student-records-for-test"
