...
<body>
	...
	<div id="frameBodyWrapper" class="container theme-showcase">
		...
		<div id="moreOptionsDiv" class="well well-plain"
			<% if(data.courses.size() == 0){ %> style="display: none;" <% } %>>
			<div class="row">
				<div class="col-md-3">
					<div class="checkbox">
						<input id="displayArchivedCourses_check" type="checkbox"
							<%if(data.displayArchive){%> checked="checked" <%}%>> <label
							for="displayArchivedCourses_check">Display Archived
							Courses</label>
					</div>
				</div>
			</div>
			<form class="form-horizontal" role="form">
				<div class="row">
					<div class="col-sm-3">
						...
						<br>
						<div class="checkbox">
							<input type="checkbox" value="" id="course_all"> <label
								for="course_all"><strong>Select all</strong></label>
						</div>
						<br>
						<%
                                int courseIdx = -1;
						...
						<div class="checkbox">
							...
							<label for="course_check-<%=courseIdx %>"> [<%=course.id%>]
								: <%=PageData.sanitizeForHtml(course.name)%>
							</label>
						</div>
						<%
                                    }
					...
					<div class="col-sm-3">
						...
						<div class="checkbox" style="display: none;">
							<input type="checkbox" value="" id="section_all"> <label
								for="section_all"><strong>Select all</strong></label>
						</div>
						<br>
						<div id="sectionChoices"></div>
					</div>
					...
					<div class="col-sm-3">
						...
						<br>
						<div class="checkbox" style="display: none;">
							<input id="team_all" type="checkbox"> <label
								for="team_all"><strong>Select All</strong></label>
						</div>
						<br>
						<div id="teamChoices"></div>
					</div>
		...
		<div
			class='panel <%= course.isArchived ? "panel-default" : "panel-info" %>'>
			<div class="panel-heading ajax_submit">
				<form style="display: none;" id="seeMore-<%=courseIdx%>"
					class="seeMoreForm-<%=courseIdx%>"
					action="<%=Const.ActionURIs.INSTRUCTOR_STUDENT_LIST_AJAX_PAGE%>">
					<input type="hidden" name="<%=Const.ParamsNames.COURSE_ID %>"
						value="<%=course.id%>"> <input type="hidden"
						name="<%=Const.ParamsNames.USER_ID%>"
						value="<%=data.account.googleId %>"> <input type="hidden"
						id="numStudents-<%=courseIdx%>"
						value="<%=data.numStudents.get(course.id)%>">
				</form>
				<a
					class="btn btn-default btn-xs pull-right pull-down course-enroll-for-test"
					id="enroll-<%=courseIdx%>"
					...
					class="glyphicon glyphicon-list"></span> Enroll
				</a>
				<div class='display-icon pull-right'></div>
				<strong>[<%=course.id%>] :
				</strong><%=PageData.sanitizeForHtml(course.name)%>
			</div>
