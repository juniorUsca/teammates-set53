...
<body>
	...
	<div class="container theme-showcase" id="frameBodyWrapper" role="main">
		...
		<div id="headerOperation" class="page-header">
			...
			<h1 id="headTitle">
				Ongoing Sessions<small> Total: <%=data.totalOngoingSessions%>
					...
					onclick="openAllSections(<%=data.tableCount%>)"> Open All</a> <a
					href="#" class="btn btn-warning"
					onclick="closeAllSections(<%=data.tableCount%>)"> Collapse All</a>
			</h1>
		...
		<div class="form-group">
			<a href="#" class="btn btn-link center-block"
				onclick="toggleFilter()"><span id="referenceText"> Show
					filter</span><br> <span class="glyphicon glyphicon-chevron-down"
							...
							<div class="row">
								<div class="col-md-6">
									<label for="<%=Const.ParamsNames.FEEDBACK_SESSION_STARTDATE%>"
										class="label-control"> From</label>
								</div>
							</div>
							<div class="row">
								<div class="col-md-6">
							...
							<div class="row">
								<div class="col-md-12">
									<label for="<%=Const.ParamsNames.FEEDBACK_SESSION_TIMEZONE%>"
										class="control-label"> Time Zone</label>
								</div>
							</div>
							<div class="row">
		...
		<div class="panel panel-primary">
			<ul class="nav nav-pills nav-stacked">
				<li id="pill_<%=tableIndex%>" class="active"><a href="#"
					onclick="toggleContent(<%=tableIndex%>); return false;"><span
						class="badge pull-right" id="badge_<%=tableIndex%>"
						style="display: none"><%=data.map.get(key).size()%></span><Strong><%=key%>
					</Strong></a></li>
			</ul>
			...
			<div class="table-responsive" id="table_<%=tableIndex%>">
				<table class="table table-striped dataTable">
					<thead>
						<tr>
							<th onclick="toggleSort(this,1)" class="button-sort-non">Session
								Name &nbsp; <span class="icon-sort unsorted"></span>
							</th>
							<th onclick="toggleSort(this,2,sortDate)" class="button-sort-non">Start
								Time&nbsp; <span class="icon-sort unsorted"></span>
							</th>
							<th onclick="toggleSort(this,3,sortDate)" class="button-sort-non">End
								Time&nbsp; <span class="icon-sort unsorted"></span>
							</th>
							<th onclick="toggleSort(this,4)" class="button-sort-non">Creator</th>
						</tr>
					</thead>
		...
		<div class="panel panel-primary">
			...
			<ul class="nav nav-pills nav-stacked">
				<li id="pill_<%=tableIndex%>" class="active"><a href="#"
					onclick="toggleContent(<%=tableIndex%>); return false;"><span
						class="badge pull-right" id="badge_<%=tableIndex%> style="display:none"><%=data.map.get(key).size()%></span>
