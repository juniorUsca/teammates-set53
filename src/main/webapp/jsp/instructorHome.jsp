			...
			<div class="row">
					...
					<form method="get" action="<%=data.getInstructorSearchLink()%>"
						...
						<div class="input-group">
							<input type="text" id="searchbox"
								...
								name="<%=Const.ParamsNames.SEARCH_KEY%>" class="form-control"
								data-toggle="tooltip" data-placement="top"
								placeholder="Student Name"> <span
								class="input-group-btn">
								<button class="btn btn-default" type="submit" value="Search"
									id="buttonSearch">Search</button>
							</span>
						</div>
						<input type="hidden" name="<%=Const.ParamsNames.SEARCH_STUDENTS%>"
				...
				<div class="col-md-2 instructor-header-bar">
					<a class="btn btn-primary btn-md"
						href="<%=data.getInstructorCourseLink() %>" id="addNewCourse">Add
						New Course </a>
				</div>
			</div>
		</div>
				...
				<div class="row">
					...
					<div class="col-md-9">
						<div class="btn-group pull-right" data-toggle="buttons">
							<label
								class="btn btn-default <%= data.sortCriteria.equals(Const.SORT_BY_COURSE_ID) ? "active" : "" %>"
								name="sortby" data="id" id="sortById"> <input
								type="radio"> Course ID
							</label> <label
								class="btn btn-default <%= data.sortCriteria.equals(Const.SORT_BY_COURSE_NAME) ? "active" : "" %>"
								name="sortby" data="name" id="sortByName"> <input
								type="radio" name="sortby" value="name"> Course Name
							</label> <label
								class="btn btn-default <%= data.sortCriteria.equals(Const.SORT_BY_COURSE_CREATION_DATE) ? "active" : "" %>"
								name="sortby" data="createdAt" id="sortByDate"> <input
								type="radio"> Creation Date
							</label>
						</div>
					</div>
				</div>
    ...
    %>
		<div class="panel panel-primary" id="course<%=courseIdx%>">
			<div class="panel-heading">
				<div class="row">
					<div class="col-md-6">
						<strong> [<%=courseDetails.course.id%>] : <%=PageData.sanitizeForHtml(courseDetails.course.name)%>
						</strong>
					</div>
					<div class="col-md-6">
						<span class="pull-right"> <a
							class="btn btn-primary btn-xs btn-tm-actions course-enroll-for-test"
							href="<%=data.getInstructorCourseEnrollLink(courseDetails.course.id)%>"
							title="<%=Const.Tooltips.COURSE_ENROLL%>" data-toggle="tooltip"
							data-placement="top"
							...
							disabled="disabled" <% } %>> Enroll</a> <a
							class="btn btn-primary btn-xs btn-tm-actions course-view-for-test"
							href="<%=data.getInstructorCourseDetailsLink(courseDetails.course.id)%>"
							title="<%=Const.Tooltips.COURSE_DETAILS%>" data-toggle="tooltip"
							data-placement="top"> View</a> <a
							class="btn btn-primary btn-xs btn-tm-actions course-edit-for-test"
							href="<%=data.getInstructorCourseEditLink(courseDetails.course.id)%>"
							title="<%=Const.Tooltips.COURSE_EDIT%>" data-toggle="tooltip"
							data-placement="top"> Edit</a> <a
							class="btn btn-primary btn-xs btn-tm-actions course-add-eval-for-test"
							href="<%=data.getInstructorEvaluationLinkForCourse(courseDetails.course.id)%>"
							title="<%=Const.Tooltips.COURSE_ADD_EVALUATION%>"
							data-toggle="tooltip" data-placement="top"
							<% if (!instructor.isAllowedForPrivilege(Const.ParamsNames.INSTRUCTOR_PERMISSION_MODIFY_SESSION)) {%>
							disabled="disabled" <% } %>> Add Session</a> <a
							class="btn btn-primary btn-xs btn-tm-actions course-archive-for-test"
							href="<%=data.getInstructorCourseArchiveLink(courseDetails.course.id, true, true)%>"
							title="<%=Const.Tooltips.COURSE_ARCHIVE%>" data-toggle="tooltip"
							data-placement="top"
							...
							<a
							class="btn btn-primary btn-xs btn-tm-actions course-notify-pending-comments-for-test
							...
							data-toggle="tooltip" data-placement="top"> <span
								class="badge"><%=numberOfPendingCommentsForThisCourse%></span> <span
								class="glyphicon glyphicon-comment"></span> <span
								class="glyphicon glyphicon-arrow-right"></span> <span
								class="glyphicon glyphicon-envelope"></span>
						</a> <a
							class="btn btn-primary btn-xs btn-tm-actions course-delete-for-test"
							href="<%=data.getInstructorCourseDeleteLink(courseDetails.course.id,true)%>"
							title="<%=Const.Tooltips.COURSE_DELETE%>" data-toggle="tooltip"
							data-placement="top"
			...
			<table class="table-responsive table table-striped table-bordered">
				<thead>
					<tr>
						...
						<th><span
							...
							data-toggle="tooltip" data-placement="top">Response Rate</span></th>
				...
				<tr id="session<%=sessionIdx%>">
					...
					<td><span
						title="<%=PageData.getInstructorHoverMessageForEval(edd)%>"
						data-toggle="tooltip" data-placement="top"> <%=PageData.getInstructorStatusForEval(edd)%>
					</span></td>
					<td
						...
						href="<%=data.getEvaluationStatsLink(edd.courseId, edd.name)%>">Show</a>
					</td>
				...
				<tr id="session<%=sessionIdx%>">
					...
					<td><span
						title="<%=PageData.getInstructorHoverMessageForFeedbackSession(fdb)%>"
						data-toggle="tooltip" data-placement="top"> <%=PageData.getInstructorStatusForFeedbackSession(fdb)%>
					</span></td>
					<td
						class="session-response-for-test<% 
						...
						<a oncontextmenu="return false;"
						href="<%=data.getFeedbackSessionStatsLink(fdb.courseId, fdb.feedbackSessionName)%>">Show</a>
					</td>
