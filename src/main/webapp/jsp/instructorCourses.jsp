	...
	<div class="container theme-showcase" id="frameBodyWrapper">
		...
		<div class="panel panel-primary">
			<div class="panel-body fill-plain">
				<form method="get"
					action="<%=Const.ActionURIs.INSTRUCTOR_COURSE_ADD%>"
					name="form_addcourse" class="form form-horizontal">
					<input type="hidden" id="<%=Const.ParamsNames.INSTRUCTOR_ID%>"
					...
					<div class="form-group">
						<div class="col-sm-offset-3 col-sm-9">
							<input id="btnAddCourse" type="submit" class="btn btn-primary"
								onclick="return verifyCourseData();" value="Add Course"
								tabindex="3">
						</div>
					</div>
				</form>
		...
		<table class="table table-bordered table-striped">
			<thead class="fill-primary">
				<tr>
					<th onclick="toggleSort(this,1);" id="button_sortcourseid"
						class="button-sort-none">Course ID<span
						class="icon-sort unsorted"></span>
					</th>
					<th onclick="toggleSort(this,2);" id="button_sortcoursename"
						class="button-sort-none">Course Name<span
						class="icon-sort unsorted"></span>
					</th>
					<th>Sections</th>
					<th>Teams</th>
			...
			<tr>
				...
				<td class="align-center no-print"><a
					class="btn btn-default btn-xs t_course_enroll<%=idx%>"
					...
					class="btn btn-default btn-xs t_course_view<%=idx%>"
					href="<%=data.getInstructorCourseDetailsLink(courseDetails.course.id)%>"
					data-toggle="tooltip" data-placement="top"
					title="<%=Const.Tooltips.COURSE_DETAILS%>"> View</a> <a
					class="btn btn-default btn-xs t_course_edit<%=idx%>"
					href="<%=data.getInstructorCourseEditLink(courseDetails.course.id)%>"
					data-toggle="tooltip" data-placement="top"
					title="<%=Const.Tooltips.COURSE_EDIT%>"> Edit</a> <a
					class="btn btn-default btn-xs t_course_delete<%=idx%>"
					...
					class="btn btn-default btn-xs t_course_archive<%=idx%>"
					href="<%=data.getInstructorCourseArchiveLink(courseDetails.course.id, true, false)%>"
					data-toggle="tooltip" data-placement="top"
					title="<%=Const.Tooltips.COURSE_ARCHIVE%>"> Archive</a></td>
			</tr>
			<%
                    }
                }
		...
		<table class="table table-bordered table-striped">
			<thead>
				<tr class="fill-default">
					<th onclick="toggleSort(this,1);" id="button_sortid"
						class="button-sort-none">Course ID<span
						class="icon-sort unsorted"></span>
					</th>
			...
			<tr>
				...
				<td class="align-center no-print"><a
					class="btn btn-default btn-xs" id="t_course_delete<%=idx%>"
					...
					<% InstructorAttributes instructor = data.instructors.get(course.id);
                               if (!instructor.isAllowedForPrivilege(Const.ParamsNames.INSTRUCTOR_PERMISSION_MODIFY_COURSE)) {%>
					disabled="disabled" <% } %>> Delete</a> <a
					class="btn btn-default btn-xs" id="t_course_unarchive<%=idx%>"
