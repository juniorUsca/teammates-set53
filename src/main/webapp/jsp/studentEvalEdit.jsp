...
<%@ page
	import="teammates.ui.controller.StudentEvalSubmissionEditPageData"%>
...
<body>
	...
	<nav class="navbar navbar-default navbar-fixed-top">
		<form method="post"
			action="<%=Const.ActionURIs.INSTRUCTOR_EVAL_PREVIEW%>"
			name="form_previewasstudent" class="form_preview">
			<h1 class="color_white align-center">
				Previewing Evaluation as <select
					name="<%=Const.ParamsNames.PREVIEWAS%>"
					onchange="this.form.submit()" style="font-size: 80%;">
					<%
			...
			<input type="hidden" name="<%=Const.ParamsNames.EVALUATION_NAME%>"
				value="<%=data.eval.name%>"> <input type="hidden"
				name="<%=Const.ParamsNames.COURSE_ID%>"
				value="<%=data.eval.courseId%>"> <input type="hidden"
	...
	<div class="container theme-showcase" id="frameBodyWrapper">
		...
		<div class="text-muted">
			<span class="bold">How do I choose ‘estimated contribution’
				values?</span><br> Choose ‘Equal share’ if this team member did an
			equal share of the work. ‘Equal share + 10%’ means the team member
			did 10% more than an equal share of the work.
		</div>
		<br> <br>
