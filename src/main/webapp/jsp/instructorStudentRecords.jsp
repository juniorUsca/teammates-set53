...
<body onload="readyStudentRecordsPage();">
	...
	<div id="frameBodyWrapper" class="container">
		...
		<h2><%=InstructorStudentRecordsPageData.sanitizeForHtml(data.student.name)%>'s
			Records<small class="muted"> - <%=data.courseId %></small>
		</h2>
		<br />
        ...
        %>
		<div class="modal fade" id="studentProfileMoreInfo" role="dialog"
			...
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title"><%=InstructorStudentRecordsPageData.sanitizeForHtml(data.student.name)%>'s
							Profile - More Info
						</h4>
					</div>
								...
								<tbody>
									<tr>
										...
										<td><%=data.studentProfile.shortName.isEmpty() ? "<i class='text-muted'>" + Const.STUDENT_PROFILE_FIELD_NOT_FILLED + "</i>" : data.studentProfile.shortName %>
											(<i> <%=data.studentProfile.gender.equals(Const.GenderTypes.OTHER) ?
				...
				<div class="col-md-12">
					<div class="panel panel-info">
						<div id="studentComments" class="panel-heading">
							Comments for
							<%=InstructorStudentRecordsPageData.sanitizeForHtml(data.student.name)%></div>
						...
						<div class="panel-body">
                            ...
                            %>
                                    ...
                                    %>
										...
										<div id="commentTextEdit<%=commentIdx %>"
											...
											<div class="form-group form-inline">
												<div class="form-group text-muted">You may change
													comment's visibility using the visibility options on the
													right hand side.</div>
												<a id="visibility-options-trigger<%=commentIdx%>"
													class="btn btn-sm btn-info pull-right"> <span
													class="glyphicon glyphicon-eye-close"></span> Show
													Visibility Options
												</a>
											</div>
											<div id="visibility-options<%=commentIdx%>"
													...
													<tbody>
														...
														<tr id="recipient-team<%=commentIdx%>">
															...
															<td><input class="visibilityCheckbox answerCheckbox"
																type="checkbox" value="<%=CommentRecipientType.TEAM%>"
														...
														<tr>
															...
															<td><input class="visibilityCheckbox answerCheckbox"
																type="checkbox"
																value="<%=CommentRecipientType.INSTRUCTOR%>"
											...
											<div class="col-sm-offset-5">
												<input id="commentsave-<%=commentIdx %>"
													title="Save comment"
													onclick="return submitCommentForm('<%=commentIdx %>');"
													type="submit" class="btn btn-primary"
													id="button_save_comment" value="Save"> <input
													type="button" class="btn btn-default" value="Cancel"
										...
										<input type="hidden"
											...
											value="<%=data.courseId%>"> <input type="hidden"
											name=<%=Const.ParamsNames.STUDENT_EMAIL%>
											value="<%=data.student.email %>"> <input
											type="hidden" name=<%=Const.ParamsNames.RECIPIENT_TYPE%>
											value="<%=comment.recipientType%>"> <input
											type="hidden" name=<%=Const.ParamsNames.RECIPIENTS%>
								...
								<%
                                  ...
                                  if(commentIdx == -1){
                              %>
								<li class="list-group-item list-group-item-warning">You
									don't have any comments on this student.</li>
								<%   
                                  }
                              %>
								<li class="list-group-item list-group-item-warning"
									id="comment_box" style="display: none;">
									<form method="post"
										action="<%=Const.ActionURIs.INSTRUCTOR_STUDENT_COMMENT_ADD%>"
										name="form_commentadd" class="form_comment">
										<div class="form-group form-inline">
											<div class="form-group text-muted">The default
												visibility for your comment is private. You may change it
												using the visibility options.</div>
											<a id="visibility-options-trigger<%=commentIdx%>"
												class="btn btn-sm btn-info pull-right"> <span
												class="glyphicon glyphicon-eye-close"></span> Show
												Visibility Options
												...
												<tbody>
													...
													<tr id="recipient-person<%=commentIdx%>">
														...
														<td><input
															class="visibilityCheckbox answerCheckbox centered"
															name="receiverLeaderCheckbox" type="checkbox"
															value="<%=CommentRecipientType.PERSON%>"></td>
														<td><input class="visibilityCheckbox giverCheckbox"
															type="checkbox" value="<%=CommentRecipientType.PERSON%>">
														</td>
														<td><input
													...
													<tr>
														...
														<td><input class="visibilityCheckbox answerCheckbox"
															type="checkbox"
															value="<%=CommentRecipientType.INSTRUCTOR%>"></td>
														<td><input class="visibilityCheckbox giverCheckbox"
															type="checkbox"
															value="<%=CommentRecipientType.INSTRUCTOR%>"></td>
														<td><input
										...
										<div class="col-sm-offset-5">
											<input type="submit" class="btn btn-primary"
												id="button_save_comment" value="Save"> <input
												type="button" class="btn btn-default" value="Cancel"
												onclick="hideAddCommentBox();"> <input type="hidden"
												...
												type="hidden" name=<%=Const.ParamsNames.RECIPIENT_TYPE%>
												value="<%=CommentRecipientType.PERSON%>"> <input
												type="hidden" name=<%=Const.ParamsNames.RECIPIENTS%>
												value="<%=data.student.email %>"> <input
												type="hidden"
												name=<%=Const.ParamsNames.COMMENTS_SHOWCOMMENTSTO%> value="">
											<input type="hidden"
												name=<%=Const.ParamsNames.COMMENTS_SHOWGIVERTO%> value="">
											<input type="hidden"
												name=<%=Const.ParamsNames.COMMENTS_SHOWRECIPIENTTO%>
					...
					<%
                        ...
                        %>
					<div class="well well-plain student_eval"
						id="studentEval-<%=evalIndex%>">
						<div class="text-primary">
							<h2 id="eval_name-<%=evalIndex%>">
								Evaluation Name:
								<%=InstructorStudentRecordsPageData.sanitizeForHtml(eval.name)%></h2>
						</div>
						<%
						...
						<h3>
							<span
								class="label <%=byReviewee ? "label-primary" : "label-default"%>"><%=(byReviewee ? "Result" : "Submission")%></span>
						</h3>
						<div
							class="panel <%=byReviewee ? "panel-primary" : "panel-default"%>">
							<table class="table panel-heading">
								<tr>
									<td class="col-sm-4"><%=byReviewee ? "Reviewee" : "Reviewer"%>:
										<strong><%=InstructorStudentRecordsPageData.sanitizeForHtml(data.student.name)%></strong>
									</td>
									<td class="col-sm-4">
										<div class="pull-right">
											<span data-toggle="tooltip" data-placement="top"
												title="<%=Const.Tooltips.CLAIMED%>">Claimed
												Contribution: </span>
									...
									<td class="col-sm-4">
										<div class="pull-right">
											<span data-toggle="tooltip" data-placement="top"
												title="<%=Const.Tooltips.PERCEIVED%>">Perceived
												Contribution: </span>
							...
							<table class="table">
								<tbody>
									<tr>
										<td><strong>Self evaluation: </strong><br /> <%=InstructorEvalSubmissionPageData.getJustificationAsSanitizedHtml(studentResult.getSelfEvaluation())%></td>
									</tr>
									<tr>
										<td><strong>Comments about the team: </strong><br /> <%String commentAboutTeam = 
							...
							<div class="panel-heading panel-default">
								<div style="margin-left: -5px">Feedback to others</div>
							</div>
							<% } %>
							<table class="table table-bordered table-striped">
								<thead>
									<tr class="border-top-gray <%=byReviewee ? "fill-info" : ""%>">
										<td width="15%"><strong><%=byReviewee ? "From" : "To"%>
												student</strong></td>
						...
						<div class="align-center">
							<input type="button" class="btn btn-primary"
								id="button_edit-<%=evalIndex %>" value="Edit Submission"
					...
					<div class="well well-plain student_feedback"
								...
								<div class="row <%=giverIndex == 1?"":"border-top-gray"%>">
                                    ...
                                    %>
										<div class="panel panel-info">
											<div class="panel-heading">
												Question
												<%=feedback.questions.get(singleResponse.feedbackQuestionId).questionNumber%>:
												<%=
                                        ...
                                        %>
												<ul class="list-group comment-list">
													<%
                                                        ...
                                                        %>
													<li class="list-group-item list-group-item-warning"><span
														class="text-muted">From: <%=comment.giverEmail %> [<%=comment.createdAt %>]
													</span>
														<div><%=comment.commentText.getValue() %></div></li>
													<%
                                                            }
                                                        %>
												</ul>
						...
						<div class="panel panel-info">
							<div class="panel-body">
								No feedback for
								<%=InstructorStudentRecordsPageData.sanitizeForHtml(data.student.name)%>
								found
							</div>
						</div>
						<%
						...
						<div class="panel panel-primary">
							...
							<div class="panel-body">
								...
								<div class="row <%=recipientIndex == 1?"":"border-top-gray"%>">
									<div class="col-md-2">
										<strong>To: <%=responsesGiven.getKey()%></strong>
									</div>
									<div class="col-md-10">
										<%
                                        int qnIndx = 1;
