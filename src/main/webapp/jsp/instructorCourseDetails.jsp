    ...
    <div class="container theme-showcase" id="frameBodyWrapper">
        ...
        <div class="well well-plain" id="courseInformationHeader">
            <button type="button"
                ...
                id="button_add_comment" data-toggle="tooltip" data-placement="top"
                title=""
                data-original-title="Give a comment about all students in the course"
            ...
            <div class="form form-horizontal">
                ...
                <div class="form-group">
                    <div class="align-center">
                        <input type="button" class="btn btn-primary" id="button_remind"
                            ...
                            disabled="disabled" <% } %>>
                        <form method="post"
                            action="<%=Const.ActionURIs.INSTRUCTOR_COURSE_STUDENT_LIST_DOWNLOAD%>"
                            style="display: inline;">
                            <input id="button_download" type="submit" class="btn btn-primary"
                                name="<%=Const.ParamsNames.FEEDBACK_RESULTS_UPLOADDOWNLOADBUTTON%>"
                                value=" Download Student List "> <input type="hidden"
                                name="<%=Const.ParamsNames.USER_ID%>"
                                value="<%=data.account.googleId%>"> <input type="hidden"
                                name="<%=Const.ParamsNames.COURSE_ID%>"
                                value="<%=data.courseDetails.course.id%>">
                        </form>
                        <input type="button" class="btn btn-primary"
                            id="button_deleteAllStudents" data-toggle="tooltip"
                            data-placement="top"
                            title="<%=Const.Tooltips.COURSE_DELETE_ALL_STUDENTS%>"
                            onclick="window.location.href='<%=data.getCourseDeleteAllStudentsLink()%>';"
                            value="Delete All Students of Course" tabindex="1"
                            <% if (!data.currentInstructor.isAllowedForPrivilege(Const.ParamsNames.INSTRUCTOR_PERMISSION_MODIFY_STUDENT)) { %>
                            disabled="disabled" <% }%> >
                        ...
                        <div>
                            <span class="help-block"> Non-English characters not
                                displayed properly in the downloaded file?<span class="btn-link"
                                data-toggle="modal" data-target="#studentTableWindow"
                        ...
                        <form id="csvToHtmlForm">
                            <input type="hidden" name="<%=Const.ParamsNames.COURSE_ID%>"
                                value="<%=data.courseDetails.course.id%>"> <input
                                type="hidden" name="<%=Const.ParamsNames.USER_ID%>"
                                value="<%=data.account.googleId%>"> <input type="hidden"
                                name="<%=Const.ParamsNames.CSV_TO_HTML_TABLE_NEEDED%>"
                                value=true>
                        </form>
                                ...
                                <div class="modal-content">
                                    <div class="modal-header">
                                        ...
                                        <span class="pull-left help-block"> Tips: After
                                            Selecting the table, <kbd>Ctrl + C</kbd> to COPY and <kbd>Ctrl
                                                + V</kbd> to PASTE to your Excel Workbook.
                                        </span>
        ...
        <div id="commentArea" class="well well-plain" style="display: none;">
            <form method="post"
                ...
                <div class="form-group form-inline">
                    <label style="margin-right: 24px;">Recipient: </label> <select
                        id="comment_recipient_select" class="form-control"
                        disabled="disabled">
                        <option value="<%=CommentRecipientType.COURSE%>" selected>The
                            whole class</option>
                    </select> <a id="visibility-options-trigger"
                        class="btn btn-sm btn-info pull-right"><span
                        class="glyphicon glyphicon-eye-close"></span> Show Visibility
                        Options</a>
                </div>
                <p class="form-group text-muted">The default visibility for your
                    comment is private. You may change it using the ‘show visibility
                    options’ button above.</p>
                ...
                <div style="text-align: center;">
                    <input type="submit" class="btn btn-primary"
                        ...
                        type="button" class="btn btn-default" id="button_cancel_comment"
                        value="Cancel"> <input type="hidden"
                        name=<%=Const.ParamsNames.COURSE_ID%>
                        value="<%=data.courseDetails.course.id%>"> <input
                        type="hidden" name=<%=Const.ParamsNames.RECIPIENT_TYPE%>
                        value="<%=CommentRecipientType.COURSE%>"> <input
                        type="hidden" name=<%=Const.ParamsNames.RECIPIENTS%>
                        value="<%=data.courseDetails.course.id%>"> <input
                        type="hidden" name=<%=Const.ParamsNames.COMMENTS_SHOWCOMMENTSTO%>
                        value=""> <input type="hidden"
                        name=<%=Const.ParamsNames.COMMENTS_SHOWGIVERTO%> value="">
                    <input type="hidden"
                        name=<%=Const.ParamsNames.COMMENTS_SHOWRECIPIENTTO%> value="">
                    <input type="hidden" name="<%=Const.ParamsNames.USER_ID%>"
                        value="<%=data.account.googleId%>"> <input type="hidden"
                        name="<%=Const.ParamsNames.FROM_COURSE_DETAILS_PAGE%>"
                        value="true">
                </div>
            </form>
        </div>
        ...
        <table class="table table-bordered table-striped">
            <thead class="fill-primary">
                <tr>
                    <%  int sortIdx = 1;
                        ...
                        if(hasSection) { %>
                    <th onclick="toggleSort(this, <%=sortIdx++%>);"
                        id="button_sortstudentsection" class="button-sort-none">
                        Section<span class="icon-sort unsorted"></span>
                    </th>
                    <% } %>
                    <th onclick="toggleSort(this, <%=sortIdx++%>);"
                        id="button_sortstudentteam" class="button-sort-none">Team<span
                        class="icon-sort unsorted"></span>
                    </th>
                    <th onclick="toggleSort(this, <%=sortIdx++%>);"
                        id="button_sortstudentname" class="button-sort-none">Student
                        Name<span class="icon-sort unsorted"></span>
                    </th>
                    <th onclick="toggleSort(this, <%=sortIdx++%>);"
                        id="button_sortstudentstatus" class="button-sort-none">
            ...
            <tr class="student_row" id="student<%=idx%>">
                ...
                <td class="align-center no-print"><a
                    ...
                    disabled="disabled" <% } %>> View</a> <a
                    class="btn btn-default btn-xs"
                    href="<%=data.getCourseStudentEditLink(student)%>"
                    data-toggle="tooltip" data-placement="top"
                    title="<%=Const.Tooltips.COURSE_STUDENT_EDIT%>"
                    <% if (!data.currentInstructor.isAllowedForPrivilege(Const.ParamsNames.INSTRUCTOR_PERMISSION_MODIFY_STUDENT)) { %>
                    disabled="disabled" <% } %>> Edit</a> <%
                                if(data.getStudentStatus(student).equals(Const.STUDENT_COURSE_STATUS_YET_TO_JOIN)){
                            %> <a class="btn btn-default btn-xs"
                    href="<%=data.getCourseStudentRemindLink(student)%>"
                    onclick="return toggleSendRegistrationKey()" data-toggle="tooltip"
                    data-placement="top"
                    title="<%=Const.Tooltips.COURSE_STUDENT_REMIND%>"
                    <% if (!data.currentInstructor.isAllowedForPrivilege(Const.ParamsNames.INSTRUCTOR_PERMISSION_MODIFY_STUDENT)) { %>
                    disabled="disabled" <% } %>> Send Invite</a> <%
                                }
                            %> <a class="btn btn-default btn-xs"
                    href="<%=data.getCourseStudentDeleteLink(student)%>"
                    onclick="return toggleDeleteStudentConfirmation('<%=sanitizeForJs(student.name)%>')"
                    data-toggle="tooltip" data-placement="top"
                    title="<%=Const.Tooltips.COURSE_STUDENT_DELETE%>"
                    <% if (!data.currentInstructor.isAllowedForPrivilege(Const.ParamsNames.INSTRUCTOR_PERMISSION_MODIFY_STUDENT)) { %>
                    disabled="disabled" <% } %>> Delete</a> <a
                    class="btn btn-default btn-xs"
                    href="<%=data.getStudentRecordsLink(student)%>"
                    data-toggle="tooltip" data-placement="top"
                    title="<%=Const.Tooltips.COURSE_STUDENT_RECORDS%>"> All Records</a>
                    <div class="btn-group">
                        <a class="btn btn-default btn-xs cursor-default"
                            ...
                            disabled="disabled" <% } %>> Add Comment</a> <a
                            href="javascript:;"
                            class="btn btn-default btn-xs dropdown-toggle"
                            data-toggle="dropdown"
                            <% if (!data.currentInstructor.isAllowedForPrivilege(student.section, Const.ParamsNames.INSTRUCTOR_PERMISSION_GIVE_COMMENT_IN_SECTIONS)) { %>
                            disabled="disabled" <% } %>><span class="caret"></span><span
                            class="sr-only">Add comments</span></a>
                        <ul class="dropdown-menu" role="menu" aria-labelledby="dLabel"
                            style="text-align: left;">
                            <li role="presentation"><a
                                class="t_student_details_tostudent-c<%=data.courseDetails.course.id %>.<%=idx%>"
                                role="menuitem" tabindex="-1"
                                href="<%=data.getCourseStudentDetailsLink(student)
                                        +"&"+Const.ParamsNames.SHOW_COMMENT_BOX+"=student"%>">
                                    To student: <%=sanitizeForHtml(student.name)%></a></li>
                            <li role="presentation"><a
                                class="t_student_details_toteam-c<%=data.courseDetails.course.id %>.<%=idx%>"
                                role="menuitem" tabindex="-1"
                                href="<%=data.getCourseStudentDetailsLink(student)
                            ...
                            <li role="presentation"><a
                                class="t_student_details_tosection-c<%=data.courseDetails.course.id %>.<%=idx%>"
                                role="menuitem" tabindex="-1"
                                href="<%=data.getCourseStudentDetailsLink(student)
